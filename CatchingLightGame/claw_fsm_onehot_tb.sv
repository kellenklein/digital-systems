...
module claw_fsm_onehot_tb;
    ...
    initial begin
        $display(" time   idle swp res shw | start grab tzero hit | obj_ld sweep_en tmr_ld tmr_en | +sc -sc +att clr_sc clr_att");
        $monitor("%4t    %0b    %0b   %0b   %0b |   %0b     %0b    %0b    %0b  |   %0b      %0b      %0b      %0b   |  %0b   %0b   %0b     %0b      %0b",
                 $time,
                 dut.idle,        // one-hot state bits inside dut
                 dut.sweep,
                 dut.result,
                 dut.show,
                 start_pulse,
    ...
    // Stimulus (same scenarios as original claw_fsm_tb)
    initial begin
        // Initial values
        rst_n          = 0;
        start_pulse    = 0;
        ...
        #50;
        $display("Yahoo! test bench for ONE-HOT FSM passed.");
        $stop;
    end
